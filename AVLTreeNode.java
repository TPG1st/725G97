...
public class AVLTreeNode<T extends Comparable<T>> {
   ...
   /* public AVLTreeNode<T> insert(T data) {
        ...
        calculateHeight(this);    //----------------- tillagt av mig -------------------------------

        if ((nodeHeight(left) - nodeHeight(right)) < -1) { //----------------- tillagt av mig -------------------------------
            singleRotationWithRightChild(this);
        }

        if ((nodeHeight(left) - nodeHeight(right)) > 1) { //----------------- tillagt av mig -------------------------------
            singleRotationWithLeftChild(this);
        }

        return this; //rotaion with lef or right child
    }
    ...
    */
    ...
    public AVLTreeNode<T> insert(T data) {
        if (data.compareTo(element) < 0) {
            // Lägg till till vänster om den nuvarande noden
            if (left == null) {
                left = new AVLTreeNode<>(data);
            } else {
                left = left.insert(data);
            ...
            // Lägg till till höger om den nuvarande noden
            if (right == null) {
                right = new AVLTreeNode<>(data);
            } else {
                right = right.insert(data);
        ...
        // Uppdatera höjden för den nuvarande noden
        calculateHeight(this);

        // Balansera trädet om det är nödvändigt
        ...
        return this; // Inget rotationskrav
    }
